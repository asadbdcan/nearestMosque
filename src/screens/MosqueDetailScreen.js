...
import { findRealWebsite, cityFromAddress } from '../services/findWebsite';
...
export default function MosqueDetailScreen({ route, navigation }) {
  ...
  // The URL we actually scraped (may differ from details.website if we
  // discovered a real site via search to replace a social-only URL).
  const [scrapedUrl, setScrapedUrl] = useState(null);
  const [scrapedHostNote, setScrapedHostNote] = useState(null);
  ...
  const loadFromWebsite = useCallback(async (websiteUrl, hostNote = null) => {
    setSalahLoading(true);
    setSalahError(null);
    setScrapedUrl(websiteUrl);
    setScrapedHostNote(hostNote);
    try {
  ...
  const loadDetailsAndTimes = useCallback(async () => {
    ...
    const site = classifyWebsite(resolved.website);

    // Case 1: We have a real website Google gave us — try to scrape it.
    if (site.kind === 'website' && site.scrapable) {
      await loadFromWebsite(site.url);
      return;
    }

    // Case 2: Google only has a Facebook/social URL or none. Search the
    // web for a real homepage we can scrape.
    setSalahLoading(true);
    setSalahError(null);
    try {
      const found = await findRealWebsite({
        mosqueName: resolved.name || mosque.name,
        city: cityFromAddress(resolved.address || mosque.address),
        excludeHost: site.host || '',
      });
      if (found?.url) {
        await loadFromWebsite(
          found.url,
          `discovered via web search (Google had only ${site.kind === 'none' ? 'no website' : site.label})`
        );
        return;
      }
      // Nothing found.
      setSalahLoading(false);
      setSalahError(
        site.kind === 'none' || site.kind === 'invalid'
          ? "This mosque doesn't have a website on file and we couldn't find one via search."
          : `Google only has this mosque's ${site.label} page, and we couldn't find a separate website to read prayer times from. Open ${site.label} above to view their announcements.`
      );
    } catch (e) {
      setSalahLoading(false);
      setSalahError(e.message);
      ...
      <View style={styles.section}>
        ...
        {loadingDetails || salahLoading ? (
          ...
          <View>
            ...
            <Text style={styles.sourceMicro}>
              Confidence: {salah.confidence}. Times read directly from {prettyHost(salah.source)}
              {scrapedHostNote ? ` (${scrapedHostNote})` : ''} — tap "Website" above to verify.
            </Text>
          </View>
        ) : (
